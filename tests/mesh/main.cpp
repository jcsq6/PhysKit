...
void test_overlap_sphere_sphere_mesh()
{
    ...
    bounding_sphere far_var{.center = vec3{10.0, 0.0, 0.0} * m, .radius = 0.5 * m};
    CHECK(msh->overlap_sphere(far_var).empty());
...
void test_overlap_sphere_instance_translated_and_rotated()
{
    ...
    bounding_sphere far_var{.center = vec3{0.0, 0.0, 0.0} * m, .radius = 1.0 * m};
    CHECK(inst.overlap_sphere(far_var).empty());

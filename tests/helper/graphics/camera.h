...
GRAPHICS_EXPORT namespace graphics
{
    ...
    struct kf
    {
        ...
        struct look_at_t
        {
            physkit::vec3<mp_units::si::metre, float> target;
        };
        /// @brief Explicit quaternion orientation.
        struct orient_t
        {
            physkit::quat<mp_units::one, float> rot;
        };
        /// @brief Direction orientation: camera faces a normalized direction vector.
        struct facing_t
        {
            physkit::vec3<mp_units::one, float> dir;
        };
        ...
        static kf make_dir(const physkit::vec3<mp_units::one, float> &d) { return kf{facing_t{d}}; }
        ...
        static kf make_pos(const physkit::vec3<mp_units::si::metre, float> &p) { return kf{p}; }
        ...
        auto &&dir(this auto &&self, const physkit::vec3<mp_units::one, float> &d)
        {
        ...
        /// @return Reference to this keyframe for method chaining.
        auto &&pos(this auto &&self, const physkit::vec3<mp_units::si::metre, float> &p)
        {
            self.M_point = p;
            return std::forward<decltype(self)>(self);
        }
        ...
        /// @return Reference to this keyframe for method chaining.
        auto &&orient(this auto &&self, const physkit::quat<mp_units::one, float> &rot)
        {
        ...
        auto &&transition(this auto &&self,
                          mp_units::quantity<mp_units::si::second, float> duration)
        {
            self.M_dt = duration;
            return std::forward<decltype(self)>(self);
        }
        ...
        /// @param target The world-space position to look at.
        /// @return Reference to this keyframe for method chaining.
        auto &&look_at(this auto &&self, const physkit::vec3<mp_units::si::metre, float> &target)
        {
    ...
    private:
        std::optional<physkit::vec3<mp_units::si::metre, float>> M_point;
        std::optional<std::variant<facing_t, orient_t, look_at_t>> M_orient;
        mp_units::quantity<mp_units::si::second, float> M_dt{0.0f * mp_units::si::second};

        kf(physkit::vec3<mp_units::si::metre, float> point) : M_point{point} {}
    ...
    /// both position and orientation over time using cubic Hermite splines (or simpler
    /// interpolation). If no orientation is given, the user will be free to look around normally.
    ///
    ...
    /// @note Keyframes without explicit orientation will derive orientation from movement
    /// direction.
    ...
    public:
        ...
        struct pose
        {
            physkit::vec3<mp_units::si::metre, float> pos; ///< Camera position in world space
            physkit::quat<mp_units::one, float> rot;       ///< Camera orientation as a quaternion
        };
        ...
        [[nodiscard]] pose at(mp_units::quantity<mp_units::si::second, float> time)
        {
            ...
            if (time - M_start_time > M_end_time)
            {
                ...
                case reverse:
                    ...
                    time =
                        M_start_time + fmod(time - M_start_time, 2 * (M_end_time - M_start_time));
            ...
            return {.pos = to_physkit_vector<mp_units::si::metre, float>(
                        M_pos_track.at(time.numerical_value_in(mp_units::si::second))),
                    .rot = to_physkit_quaternion<mp_units::one, float>(
                        M_orient_track.at(time.numerical_value_in(mp_units::si::second)))};
        }
    ...
    private:
        ...
        second_t M_end_time = -1.f * mp_units::si::second;
        ...
        void compile_pos()
        {
            ...
            {
                arrayAppend(t, time.numerical_value_in(mp_units::si::second));
                arrayAppend(p,
                            to_magnum_vector<mp_units::si::metre, float>(keyframe.point().value()));
            }
        ...
        void compile_orient()
        {
            ...
            const float t_end = M_duration.numerical_value_in(mp_units::si::second);
            ...
            for (auto &[time, keyframe] :
                 M_kfs |
                     std::views::filter([](auto &kv) { return kv.second.orient().has_value(); }))
            {
                const float t_sec = time.numerical_value_in(mp_units::si::second);
                arrayAppend(t, t_sec);
            ...
            for (std::size_t i = 0; i < n; ++i)
            {
                ...
                arrayAppend(orient_format, {t[i], Math::CubicHermiteQuaternion<float>{
                                                      in_tangent, q[i], out_tangent}});
            }
            ...
            return std::visit(
                [&](auto &&o) -> Quaternion
                {
                    ...
                    if constexpr (std::same_as<T, kf::orient_t>)
                        return to_magnum_quaternion<mp_units::one, float>(o.rot).normalized();
                    else if constexpr (std::same_as<T, kf::facing_t>)
                        return look_rotation(to_magnum_vector<mp_units::one, float>(o.dir));
                    else if constexpr (std::same_as<T, kf::look_at_t>)
                        return look_rotation(
                            to_magnum_vector<mp_units::si::metre, float>(o.target) -
    ...
    {
    public:
        constexpr static float near_var = 0.01f;
        constexpr static float far_var = 1000.0f;

        constexpr static auto up_axis = Vector3::yAxis();

        template <class Transform>
        explicit camera(SceneGraph::Scene<Transform> &scene,
                        mp_units::quantity<mp_units::si::degree, float> fov,
                        const physkit::vec3<mp_units::si::metre, float> &position,
                        const physkit::vec3<mp_units::one, float> &dir, const Vector2i &window_size,
                        const Vector2i &viewport_size,
                        mp_units::quantity<mp_units::si::metre / mp_units::si::second, float>
                            speed = 4.0f * mp_units::si::metre / mp_units::si::second)
            : M_fov{fov}, M_window_size{window_size},
              M_pos{to_magnum_vector<mp_units::si::metre, float>(position)}, M_speed{speed}
        {
            ...
            M_cam->setAspectRatioPolicy(SceneGraph::AspectRatioPolicy::Extend)
                .setProjectionMatrix(Matrix4::perspectiveProjection(
                    ...
                    Vector2(viewport_size).aspectRatio(), near_var, far_var))
                .setViewport(viewport_size);

            set_view(dir);
        }

        void set_view(const physkit::vec3<mp_units::one, float> &d)
        {
            const Vector3 dir = to_magnum_vector<mp_units::one, float>(d);
        ...
        void look_at(const physkit::vec3<mp_units::si::metre, float> &pos)
        {
            set_view((pos - to_physkit_vector<mp_units::si::metre, float>(M_pos)) /
                     (1.0f * mp_units::si::metre));
        }
        ...
        {
            M_pos += to_magnum_vector<mp_units::si::metre, float>(delta);
            mark_dirty();
        }

        void move(float forward, float right, float up,
                  physkit::quantity<mp_units::si::second, float> dt)
        {
            Vector3 delta{0.0f};
        ...
        void rotate(physkit::quantity<mp_units::si::radian, float> d_yaw,
                    physkit::quantity<mp_units::si::radian, float> d_pitch)
        {
            float yaw = d_yaw.numerical_value_in(mp_units::si::radian);
        ...
        void pointer_move(Platform::Application::PointerMoveEvent &event, bool drag)
        {
            ...
            if (drag)
                rotate((dx * sx) * mp_units::si::radian, (dy * sy) * mp_units::si::radian);
            else
                rotate((-dx * sx) * mp_units::si::radian, (-dy * sy) * mp_units::si::radian);
        }
        ...
        /// @note This overrides manual camera controls. To restore manual control, set an empty
        /// track.
        ...
        bool update(physkit::quantity<mp_units::si::second> t)
        {
            auto update = [&]
            {
            ...
            if (!M_track.empty() && !M_track.released(t))
            {
                ...
                if (!M_track.freelook()) M_rot = to_magnum_quaternion<mp_units::one, float>(rot);
                return update(), true;
            }
        ...
        void draw(SceneGraph::DrawableGroup3D &drawables, physkit::quantity<mp_units::si::second> t)
        {
            update(t);
    ...
    private:
        ...
        physkit::quantity<mp_units::si::degree, float> M_fov{};
        physkit::quantity<mp_units::si::metre / mp_units::si::second, float> M_speed{};

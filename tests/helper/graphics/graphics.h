...
GRAPHICS_EXPORT namespace graphics
{
    ...
    public:
        ...
        auto &&read_file(this auto &&self, std::string_view path)
        {
            ...
            for (const obj &obj : config.objects)
            {
                    ...
                    obj.mesh.visit(
                        [&](auto &&mesh_desc)
                        {
                            ...
                            if constexpr (std::same_as<T, box_type>)
                                mesh_map[mesh_name] =
                                    physkit::mesh::box(physkit::vec3{mesh_desc.half_extents[0],
                                                                     mesh_desc.half_extents[1],
                                                                     mesh_desc.half_extents[2]} *
                                                       m);
                            else if constexpr (std::same_as<T, pyramid_type>)
                                mesh_map[mesh_name] = physkit::mesh::pyramid(
                ...
                self.M_objects.emplace_back(
                    physkit::object_desc{obj.type}
                        ...
                        .with_orientation(
                            physkit::quat<mp_units::one>{obj.orientation[0], obj.orientation[1],
                                                         obj.orientation[2], obj.orientation[3]})
                        .with_ang_vel(physkit::vec3{obj.angular_velocity[0],
            ...
            if (config.look_at)
            {
                self.M_look_at = physkit::vec3{(*config.look_at)[0], (*config.look_at)[1],
                                               (*config.look_at)[2]} *
                                 m;
            ...
            if (config.cam_pos)
            {
                self.M_cam_pos = physkit::vec3{(*config.cam_pos)[0], (*config.cam_pos)[1],
                                               (*config.cam_pos)[2]} *
                                 m;
            ...
            if (config.window_size)
            {
                ...
                std::println("  Window size: {}x{}", self.M_window_size->x(),
                             self.M_window_size->y());
            }
            if (config.drag)
            {
            ...
            : M_args(args)
        {
            ...
            argparse::ArgumentParser parser("PhysKit Configurable Demo",
            ...
            parser.add_argument("--window-size", "-w")
                .default_value(
                    std::format("{},{}", default_window_size.x(), default_window_size.y()))
            ...
            if (read_config)
                parser.add_argument("--config", "-c")
                    .help(
                        "Path to JSON config file for the world (CLI arguments will override JSON "
                        "config)");
            ...
            auto read_vec =
                []<int size, typename Rep = double>(const std::string &s,
                                                    std::integral_constant<int, size> size_constant,
            ...
            try
            {
                ...
                auto cam_pos_val = read_vec(parser.get<std::string>("--cam-pos"),
                                            std::integral_constant<int, 3>{}) *
                                   m;
                auto cam_dir_val = read_vec(parser.get<std::string>("--cam-dir"),
                                            std::integral_constant<int, 3>{});
                auto title_val = parser.get<std::string>("--title");
                auto window_size_val =
                    to_magnum_vector<one, int>(read_vec(parser.get<std::string>("--window-size"),
                                                        std::integral_constant<int, 2>{}, int{}));

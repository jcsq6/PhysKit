...
private:
                                               ...
                                               mp_units::quantity<si::second> fuse)
    {
        ...
        auto pos = (*co_await get_rigid(gh))->pos();
        delete gfx;
        co_await add_task(spawn_debris(self, pos, 14, 7.0, 8.0, 0.12f, {0.9f, 0.45f, 0.1f}));
        co_return pos;
    }
                                                 ...
                                                 float debris_size, Color3 debris_color)
    {
        ...
        auto desired_speed = (*co_await get_rigid(handle))->vel().norm();

        // NOLINTNEXTLINE
        auto end = [&](this auto, auto &obj) -> task<vec3<si::metre>>
        {
            delete gfx;
            co_await add_task(
                              ...
                              vec3<si::metre> target_b, vec3<si::metre> target_c)
    {
        ...
        auto apex_pos = (*co_await get_rigid(carrier_h))->pos();
        delete carrier_gfx;

        // Flash at split point
        co_await add_task(spawn_debris(self, apex_pos, 6, 3.0, 2.0, 0.06f, {1.0f, 0.9f, 0.3f}));

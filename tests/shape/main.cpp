...
{
    shape s;
    CHECK(s.stored_type() == shape::type::box);
...
void test_shape_from_sphere()
{
    sphere sph(2.0 * m);
    shape s(sph);
    CHECK(s.stored_type() == shape::type::sphere);
...
void test_shape_from_mesh()
{
    ...
    shape s(msh);
    CHECK(s.stored_type() == shape::type::mesh);
...
void test_shape_copy()
{
    ...
    CHECK(s2.stored_type() == shape::type::sphere);
    ...
    shape s3(bx);
    s2 = s3;
    CHECK(s2.stored_type() == shape::type::box);

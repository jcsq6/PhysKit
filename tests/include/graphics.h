...
public:
    ...
    void rotate(physkit::quantity<physkit::si::radian, float> angle, const physkit::vec3<physkit::one> &axis)
    {
...
{
public:
    explicit physics_obj(std::derived_from<gfx_obj_base> auto &parent, std::unique_ptr<physkit::object> obj = nullptr)
...
{
public:
    explicit instanced_drawables(std::derived_from<gfx_obj_base> auto &parent, SceneGraph::DrawableGroup3D *group,
...
{
public:
    explicit instanced_drawable(std::derived_from<gfx_obj_base> auto &parent, instanced_drawables &group)

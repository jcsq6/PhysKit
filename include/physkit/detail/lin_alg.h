...
template <Quantity Q> requires(!std::is_const_v<Q>)
...
public:
    ...
    constexpr auto operator<=>(Q other) const noexcept
    {
        return static_cast<Q>(*this) <=> other;
    }
...
template <typename T> requires(requires { typename std::remove_cvref_t<T>::value_type; std::remove_cvref_t<T>::ref; })
...
template <Quantity Q>
constexpr auto operator*(quantity_ref<Q> a, auto scalar) { return static_cast<Q>(a) * scalar; }
template <Quantity Q>
constexpr auto operator/(quantity_ref<Q> a, auto scalar) { return static_cast<Q>(a) / scalar; }
template <Quantity Q>
constexpr auto operator+(quantity_ref<Q> a, Q other) { return static_cast<Q>(a) + other; }
template <Quantity Q>
constexpr auto operator-(quantity_ref<Q> a, Q other) { return static_cast<Q>(a) - other; }

template <Quantity Q>
constexpr auto operator*(auto scalar, quantity_ref<Q> b) { return scalar * static_cast<Q>(b); }
template <Quantity Q>
constexpr auto operator/(auto scalar, quantity_ref<Q> b) { return scalar / static_cast<Q>(b); }
template <Quantity Q>
constexpr auto operator+(auto scalar, quantity_ref<Q> b) { return scalar + static_cast<Q>(b); }
...
public:
    ...
    constexpr decltype(auto) base(this auto &&self) { return std::forward_like<decltype(self)>(self.data); }

    constexpr auto x(this auto &&self) { return detail::to_reference<decltype(self)>(std::forward<decltype(self)>(self).base().x()); }
    constexpr auto y(this auto &&self) { return detail::to_reference<decltype(self)>(std::forward<decltype(self)>(self).base().y()); }
    constexpr auto z(this auto &&self) { return detail::to_reference<decltype(self)>(std::forward<decltype(self)>(self).base().z()); }
    constexpr auto w(this auto &&self) { return detail::to_reference<decltype(self)>(std::forward<decltype(self)>(self).base().w()); }

    constexpr auto rows() const { return base().rows(); }
    constexpr auto cols() const { return base().cols(); }

    constexpr auto begin(this auto &&self) requires(eigen_type::IsVectorAtCompileTime == 1) { return std::forward_like<decltype(self)>(self.data).begin(); }
    constexpr auto end(this auto &&self)   requires(eigen_type::IsVectorAtCompileTime == 1) { return std::forward_like<decltype(self)>(self.data).end(); }

    auto transpose() const { return unit_mat{base().transpose()}; }
    auto determinant() const requires(_rows == _cols) { return base().determinant() * mp_units::pow<_cols>(ref); }
    auto inverse() const requires(_rows == _cols)
    {
    ...
    {
        return base().squaredNorm() * ref * ref;
    }

    auto norm() const
    {
        return base().norm() * ref;
    }
    ...
    constexpr auto operator*(auto scalar) const requires(std::is_arithmetic_v<decltype(scalar)>)
    {
    ...
    constexpr auto &operator*=(auto scalar) requires(std::is_arithmetic_v<decltype(scalar)>)
    {
        base() *= scalar;
        return *this;
    }
    ...
    constexpr static auto ones()
    {
        return unit_mat{eigen_type::Ones()};
    }

    constexpr static auto zero()
    {
        return unit_mat{eigen_type::Zero()};
    }

    constexpr static auto identity() requires(_rows == _cols)
    {
        return unit_mat{eigen_type::Identity()};
    }

    constexpr static auto random()
    {
        return unit_mat{eigen_type::Random()};
    }
